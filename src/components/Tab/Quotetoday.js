...
function Quotetoday() {
  ...
  const getenquiryadd = async () => {
    ...
    if (res.status === 200) {
      setenquiryflwdata(res.data?.quotefollowup.filter((item) => item.nxtfoll === today));
      setSearchResults(res.data?.quotefollowup.filter((item) => item.nxtfoll === today)); // Update the searchResults state with the full data
    }
  };
  let i = 0;
